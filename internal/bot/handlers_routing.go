...
func (b *Bot) handleCallback(ctx *th.Context, query telego.CallbackQuery) error {
	...
	if strings.HasPrefix(data, "move_") {
		...
		if len(parts) >= 4 && parts[0] == "move" {
			...
			if err1 == nil && err2 == nil {
				b.handleMoveClientImpl(chatID, messageID, fromPanelName, fromInboundID, clientIndex)
				return nil
			}
		}
	}
	...
	if strings.HasPrefix(data, "move_confirm_") {
		...
		if len(parts) >= 5 {
			...
			if len(numericIndices) == 3 {
				...
				if err1 == nil {
					b.handleMoveClientConfirmImpl(chatID, messageID, fromPanelName, fromInboundID, clientIndex, toPanelName, toInboundID)
					return nil
				}
			}
		}
...
// Deprecated: functions moved to handlers_clients.go (impl versions)

// handleMoveClient moved to handlers_clients.go as handleMoveClientImpl

// handleMoveClientConfirm moved to handlers_clients.go as handleMoveClientConfirmImpl
