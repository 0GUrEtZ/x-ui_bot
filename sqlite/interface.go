...
// TrafficRecord represents a traffic snapshot for a client on a specific date
...
type Storage interface {
	...
	// Traffic history
	SaveTrafficSnapshot(date string, email string, totalBytes uint64) error
	GetTrafficHistory(days int) ([]TrafficRecord, error)
	CleanupOldTraffic(olderThanDays int) error
