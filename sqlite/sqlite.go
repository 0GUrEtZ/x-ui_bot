...
func (s *SQLiteStorage) initialize() error {
	...
	CREATE INDEX IF NOT EXISTS idx_traffic_date ON traffic_history(date);
	CREATE INDEX IF NOT EXISTS idx_traffic_email ON traffic_history(email);
	`
...
// SaveTrafficSnapshot saves or updates a traffic snapshot for a specific date and email
func (s *SQLiteStorage) SaveTrafficSnapshot(date string, email string, totalBytes uint64) error {
	_, err := s.db.Exec(`
		INSERT OR REPLACE INTO traffic_history (date, email, total_bytes)
		VALUES (?, ?, ?)`,
		date, email, totalBytes,
	)
	return err
}

// GetTrafficHistory returns traffic records for the last N days
func (s *SQLiteStorage) GetTrafficHistory(days int) ([]TrafficRecord, error) {
	cutoffDate := time.Now().AddDate(0, 0, -days).Format("2006-01-02")

	rows, err := s.db.Query(`
		SELECT date, email, total_bytes, created_at
	...
	for rows.Next() {
		...
		err := rows.Scan(&record.Date, &record.Email, &record.TotalBytes, &record.CreatedAt)
		if err != nil {
			return nil, err
		}
...
// CleanupOldTraffic removes traffic records older than the specified number of days
func (s *SQLiteStorage) CleanupOldTraffic(olderThanDays int) error {
